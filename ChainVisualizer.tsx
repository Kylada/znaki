...
export const ChainVisualizer: React.FC = () => {
  ...
  // Am I the one who proposed resolution, or the one who needs to confirm?
  const iAmInitiator = resolutionPending?.initiatorId === localPlayerId;
  const iAmConfirmer = resolutionPending && !iAmInitiator;
    ...
    <div className="bg-gray-900/90 border border-gray-700 rounded-lg p-2">
      ...
      {chain.length > 0 && (
        <div className="flex gap-1 mb-2">
          {!resolutionPending ? (
            // No pending resolution — both players see resolve buttons
            <>
              <button
            ...
            // I proposed — waiting for opponent to confirm
            <div className="flex flex-col gap-2 flex-1 bg-gray-800 p-2 rounded border border-yellow-600/50">
              ...
              <button
                className="bg-gray-700 hover:bg-gray-600 text-white text-xs px-2 py-1.5 rounded self-end"
