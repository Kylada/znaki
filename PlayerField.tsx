...
const ZoneSlot: React.FC<{
  ...
  // Read reactive state via selector — never destructure action functions
  // that may be undefined due to the malformed store interface.
  ...
  const handleCardClick = (cardId: string) => {
    // Always call via getState() so we get real runtime functions
          ...
          return (
            ...
            >
              <Card
                ...
                onContextMenu={(e) => {
                  e.preventDefault();
                  useGameStore.getState().openContextMenu(card.instanceId, e.clientX, e.clientY);
                }}
              />
              {isAttacker && (
