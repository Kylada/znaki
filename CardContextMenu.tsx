...
export const CardContextMenu: React.FC = () => {
  ...
  // Unstack: detach this card from whatever it's stacked under, placing it freely in the same zone
  const handleUnstack = () => {
    useGameStore.getState().stackCardOnCard(contextMenuCardId, card.fieldStackedUnder!, 'above');
    // That would put it above — instead just clear fieldStackedUnder by moving within zone
    // Actually we want to simply free it: set fieldStackedUnder to undefined in place
    // We do this by moving the card to its own zone (no-op zone move but clears stack)
    useGameStore.setState(state => ({
      players: {
        ...state.players,
        [cardPlayerId]: {
          ...state.players[cardPlayerId],
          cards: state.players[cardPlayerId].cards.map(c =>
            c.instanceId === contextMenuCardId
              ? { ...c, fieldStackedUnder: undefined }
              : c
          )
        }
      },
      log: [...state.log, `${card.template.name} откреплена от стека`]
    }));
    useGameStore.getState().syncBoardState();
    closeContextMenu();
  };
    ...
    <>
      ...
      >
        ...
        <div className="px-3 py-1.5 text-yellow-400 font-bold border-b border-gray-700 text-xs truncate">
          ...
          {card.fieldStackedUnder && <span className="text-gray-500 ml-1">(в стеке)</span>}
        </div>
        ...
        {/* Unstack — only for cards that are stacked under something */}
        {card.fieldStackedUnder && (
          <button
            className="w-full text-left px-3 py-1.5 hover:bg-orange-900 text-orange-300"
